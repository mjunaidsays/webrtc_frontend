...
export default function ConferenceRoom() {
  ...
  useEffect(() => {
    ...
    async function startAudioStreaming() {
      try {
        ...
        ws = new window.WebSocket(`ws://localhost:8001/ws/audio/${roomId}`);
  ...
  const handleEndMeeting = async () => {
    ...
    try {
      await fetch(`http://localhost:8001/api/meetings/${roomId}/end`, {
        method: 'POST'
      });
    } catch (error) {
  ...
  const pollForSummary = () => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const res = await fetch(`http://localhost:8001/api/insights/${roomId}/view`);
        if (res.ok) {
